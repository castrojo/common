...
- Push to the head owner's URL, never a shared `fork` remote: `git push https://github.com/<headRepositoryOwner>/<repo>.git HEAD:<headRefName> --force-with-lease=<headRefName>:<headRefOid>`, both values from `--json`. An empty lease OID creates stray branches. A fork push can 403 despite `maintainerCanModify: true` (contribute#633): record blocked(author rebase).
...
- Fork heads get `action_required` runs: approve each with `gh api -X POST repos/<o>/<r>/actions/runs/<id>/approve`. Approval starts CI; it does not pass it. An empty rollup means no run was approved yet (common#1180).
